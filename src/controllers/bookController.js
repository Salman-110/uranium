...
const getBooksInYear= async function (req, res) {
    ...
    const getParticularBooks= async function (req, res) {
        // let booksByFunction= await BookModel.find({authorName :"Khaled" , isPublished: true  })
       

        // let booksByFunction= await BookModel.find( {
            // $or: [ {authorName : "Khaled" } , { isPublished: false } , {  "year": 2003 }]})

        // let booksByFunction= await BookModel.find().sort({totalPages: -1})

        // let booksByFunction= await BookModel.find({"year":2000}).limit(2)
